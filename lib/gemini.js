...
async function callGemini(question) {
  ...
  // Find JSON object within the text (handles thinking model preamble)
  const match = clean.match(/\{[\s\S]*\}/);
  if (match) {
    try { return JSON.parse(match[0]); } catch (_) {}
  }

  // Last resort: build a valid response from the raw text
  console.error('JSON parse failed, using raw text fallback. Raw:', clean.slice(0, 200));
  return {
    isAgro:   true,
    category: 'other',
