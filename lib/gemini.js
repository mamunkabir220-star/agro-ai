...
async function callGemini(question) {
  const res = await fetch(GEMINI_URL, {
    ...
    headers: {
      ...
      'x-goog-api-key':  GEMINI_API_KEY,  // Key in header, not URL — per Google docs
    },
    body: JSON.stringify({
...
export async function getProductResponse(question, rateInfo = {}) {
  ...
  // Add question number context so AI knows where in funnel we are
  const qNum    = (rateInfo.used || 0) + 1;
  const context = `[Question ${qNum} of 15. User has ${rateInfo.remaining || 0} questions remaining today${rateInfo.isBonus ? ' (BONUS zone — final push!)' : ''}]`;
  const fullQ   = `${context}\n\nUser: ${question}`;

  let parsed;
  try {
    const prodUrl = `${GEMINI_BASE}/${GEMINI_MODELS[0]}:generateContent`;
    ...
    console.error('Product Gemini error:', err.message);
    return {
      success:  false,
      isAgro:   true,
      response: language === 'bn'
  ...
  // Cache product response (shorter — 7 days)
  if (parsed?.answer) {
    try {
