/**
 * Agro Assistant AI Engine — Gemini-first, fully robust
 *
...
const SYSTEM_PROMPT = `You are Agro Assistant, an expert AI agricultural advisor for farmers in Bangladesh.
  ...
  for (const model of GEMINI_MODELS) {
    try {
      ...
      if (data.error?.code === 429) {
        console.warn(`[Agro Assistant] ${model} rate limited — trying next model`);
      ...
      console.log(`[Agro Assistant] Responded via ${model}`);
...
// ─────────────────────────────────────────────
const PRODUCT_SYSTEM_PROMPT = `You are Agro Assistant Sales Assistant for agro.com.bd — Bangladesh's leading agricultural marketplace.
...
export async function getProductResponse(question, rateInfo = {}) {
  ...
  for (const model of GEMINI_MODELS) {
    try {
      ...
      if (data.error?.code === 429) {
        console.warn(`[Agro Assistant Product] ${model} rate limited — trying next`);
        lastError = new Error(`${model} 429`);
        continue;
      }
      ...
      console.log(`[Agro Assistant Product] Responded via ${model}`);
      break;
